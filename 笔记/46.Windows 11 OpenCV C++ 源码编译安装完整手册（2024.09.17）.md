# Windows 11 OpenCV C++ 源码编译安装完整手册（2024.09.17）

---

## 1. 环境准备

### 1.1 安装 Visual Studio 2022

1. 访问 [Visual Studio 官网](https://visualstudio.microsoft.com/zh-hans/)
2. 下载 **Visual Studio 2022 Community**
3. 运行安装程序，选择以下工作负载：

```
☑️ 使用 C++ 的桌面开发
```

### 1.2 安装 CMake

1. 访问 [CMake 官网](https://cmake.org/download/)
2. 下载 **cmake-3.28.x-windows-x86_64.msi**（同学们选择最新稳定版就可以了）
3. 运行安装程序：

```
安装选项中选择：
◉ Add CMake to the system PATH for all users
```

4. 验证安装（打开命令提示符）：
```cmd
cmake --version
```
应显示类似：`cmake version 3.28.x`

### 1.3 安装 Git（可选但推荐）

1. 访问 [Git 官网](https://git-scm.com/download/win)
2. 下载并安装 64-bit Git for Windows
3. 安装时保持默认选项即可

---

## 2. 下载源码

### 2.1 创建工作目录

打开命令提示符（**以管理员身份运行，需要这样做**），执行：

```cmd
:: 创建 OpenCV 工作目录
mkdir C:\opencv
cd C:\opencv
```

### 2.2 下载 OpenCV 源码

**方法一：使用 Git 克隆（推荐）**

```cmd
:: 克隆 OpenCV 主仓库
git clone https://github.com/opencv/opencv.git

:: 克隆 OpenCV 扩展模块
git clone https://github.com/opencv/opencv_contrib.git
```

### 2.3 创建构建目录

```cmd
cd C:\opencv
mkdir build
```

**此时目录结构应为：**
```
C:\opencv\
├── opencv\           # OpenCV 源码
├── opencv_contrib\   # 扩展模块源码
└── build\            # 构建输出目录
```

---

## 3. CMake 命令行配置

### 3.1 打开开发者命令提示符

按 `Win + S` 搜索并打开：
```
x64 Native Tools Command Prompt for VS 2022
```

> ⚠️ **重要**：**同学们注意，必须使用此命令提示符**

### 3.2 进入构建目录

```cmd
cd C:\opencv\build
```

### 3.3 配置（无NVIDIA显卡）

如果下载了 `opencv_contrib`，使用以下命令：

```cmd
cmake -G "Visual Studio 17 2022" -A x64 ^
    -DCMAKE_INSTALL_PREFIX=D:/Projects/DeepBule/C++Project/opencv/install ^
    -DCMAKE_BUILD_TYPE=Release ^
    -DBUILD_EXAMPLES=ON ^
    -DBUILD_opencv_world=ON ^
    -DBUILD_TESTS=OFF ^
    -DBUILD_PERF_TESTS=OFF ^
    -DBUILD_DOCS=OFF ^
    -DWITH_OPENGL=ON ^
    -DWITH_FFMPEG=ON ^
    -DOPENCV_EXTRA_MODULES_PATH=D:/Projects/DeepBule/C++Project/opencv/opencv_contrib/modules ^
    ../opencv
```

### 3.5 完整配置(工业级启用 CUDA 支持）（需要 NVIDIA 显卡）

```cmd
cmake -G "Visual Studio 17 2022" -A x64 ^
    -DCMAKE_INSTALL_PREFIX=D:/Projects/DeepBule/C++Project/opencv/install ^
    -DCMAKE_BUILD_TYPE=Release ^
    -DBUILD_EXAMPLES=ON ^
    -DBUILD_opencv_world=ON ^
    -DBUILD_TESTS=ON ^
    -DBUILD_PERF_TESTS=ON ^
    -DBUILD_DOCS=ON ^
    -DWITH_OPENGL=ON ^
    -DWITH_FFMPEG=ON ^
    -DWITH_CUDA=ON ^
    -DOPENCV_DNN_CUDA=ON ^
    -DCUDA_ARCH_BIN="8.6" ^
    -DOPENCV_EXTRA_MODULES_PATH=D:/Projects/DeepBule/C++Project/opencv/opencv_contrib/modules ^
    ../opencv
    
    
cmake -G "Visual Studio 17 2022" -A x64 -DCMAKE_INSTALL_PREFIX=D:/Projects/DeepBule/C++Project/opencv/install -DCMAKE_BUILD_TYPE=Release -DBUILD_EXAMPLES=ON -DBUILD_opencv_world=ON -DBUILD_TESTS=ON -DBUILD_PERF_TESTS=ON -DBUILD_DOCS=ON -DWITH_OPENGL=ON -DWITH_FFMPEG=ON -DWITH_CUDA=ON -DOPENCV_DNN_CUDA=ON -DCUDA_ARCH_BIN="8.6" -DOPENCV_EXTRA_MODULES_PATH=D:/Projects/DeepBule/C++Project/opencv/opencv_contrib/modules ../opencv
```

> 💡 `CUDA_ARCH_BIN` 根据显卡型号设置：**RTX 30系列用 "8.6"，RTX 40系列用 "8.9"**

### 3.6 配置成功输出

配置成功后，应看到类似输出：

```
-- General configuration for OpenCV 4.9.0 =====================================
--   Version control:               4.9.0
--
--   Platform:
--     Timestamp:                   2024-xx-xx
--     Host:                        Windows 10.0.22631 AMD64
--     CMake:                       3.28.x
--     CMake generator:             Visual Studio 17 2022
--     CMake build tool:            C:/Program Files/Microsoft Visual Studio/2022/...
--     MSVC:                        1939
--
--   CPU/HW features:
--     Baseline:                    SSE SSE2 SSE3
--     Dispatched code generation:  SSE4_1 SSE4_2 AVX AVX2 ...
--
--   C/C++:
--     Built as dynamic libs?:      YES
--     C++ standard:                11
--     ...
--
--   OpenCV modules:
--     To be built:                 core imgproc imgcodecs ... world
--     Disabled:                    ...
--     ...
--
-- Configuring done
-- Generating done
-- Build files have been written to: C:/opencv/build
```

## 4. 编译与安装

### 4.1 使用命令行编译

在 **x64 Native Tools Command Prompt for VS 2022** 中执行：

```cmd
cd C:\opencv\build

:: 编译 Release 版本
cmake --build . --config Release

:: 安装 Release 版本
cmake --build . --config Release --target INSTALL
```

### 4.2 验证编译结果

编译安装完成后，检查安装目录：

```cmd
dir C:\opencv\install\x64\vc17\bin\*.dll
dir C:\opencv\install\x64\vc17\lib\*.lib
dir C:\opencv\install\include\opencv2
```

预期目录结构：
```
C:\opencv\install\
├── include\
│   └── opencv2\         # 头文件
├── x64\
│   └── vc17\
│       ├── bin\         # DLL 动态库
│       │   ├── opencv_world490.dll      (Release)
│       └── lib\         # LIB 导入库
│           ├── opencv_world490.lib      (Release)
└── etc\                 # 配置文件（Haar 级联等）
```

---

## 5. 环境变量配置

### 5.1 使用命令行设置（需管理员权限）

打开 **管理员命令提示符**：

```cmd
:: 设置 OPENCV_DIR 环境变量
setx OPENCV_DIR "C:\opencv\install" /M

:: 添加 DLL 路径到 PATH
setx PATH "%PATH%;C:\opencv\install\x64\vc17\bin" /M
```

### 5.3 验证环境变量

**重新打开** 一个新的命令提示符窗口：

```cmd
:: 验证 OPENCV_DIR
echo %OPENCV_DIR%
:: 应输出: C:\opencv\install

:: 验证 DLL 可访问
where opencv_world490.dll
:: 应输出: C:\opencv\install\x64\vc17\bin\opencv_world490.dll
```

---

## 6. 验证安装

### 6.1 创建测试项目文件夹

```cmd
mkdir C:\opencv_test
cd C:\opencv_test
```

### 6.2 创建测试源文件

创建文件 `test_opencv.cpp`：

```cpp
#include <opencv2/opencv.hpp>
#include <iostream>

int main() {
    // 打印 OpenCV 版本
    std::cout << "================================" << std::endl;
    std::cout << "OpenCV Version: " << CV_VERSION << std::endl;
    std::cout << "================================" << std::endl;
    
    // 创建一个 500x500 的彩色图像（蓝色背景）
    cv::Mat image(500, 500, CV_8UC3, cv::Scalar(255, 100, 0));
    
    // 在图像上绘制文本
    cv::putText(image, 
                "OpenCV Works!", 
                cv::Point(100, 250),
                cv::FONT_HERSHEY_SIMPLEX, 
                1.5, 
                cv::Scalar(255, 255, 255),
                3);
    
    // 绘制一个圆
    cv::circle(image, cv::Point(250, 350), 80, cv::Scalar(0, 255, 0), -1);
    
    // 绘制矩形
    cv::rectangle(image, cv::Point(50, 50), cv::Point(150, 150), 
                  cv::Scalar(0, 0, 255), 3);
    
    // 显示图像
    cv::imshow("OpenCV Test", image);
    
    std::cout << "Image created successfully!" << std::endl;
    std::cout << "Press any key to close..." << std::endl;
    
    cv::waitKey(0);
    
    // 保存图像
    cv::imwrite("test_output.png", image);
    std::cout << "Image saved as test_output.png" << std::endl;
    
    return 0;
}
```

### 6.3 创建 CMakeLists.txt

创建文件 `CMakeLists.txt`：

```cmake
cmake_minimum_required(VERSION 3.20)
project(OpenCV_Test)

# 设置 C++ 标准
set(CMAKE_CXX_STANDARD 17)
set(CMAKE_CXX_STANDARD_REQUIRED ON)

# 查找 OpenCV
find_package(OpenCV REQUIRED)

# 打印 OpenCV 信息
message(STATUS "=================================")
message(STATUS "OpenCV version: ${OpenCV_VERSION}")
message(STATUS "OpenCV include dirs: ${OpenCV_INCLUDE_DIRS}")
message(STATUS "OpenCV libraries: ${OpenCV_LIBS}")
message(STATUS "=================================")

# 创建可执行文件
add_executable(test_opencv test_opencv.cpp)

# 链接 OpenCV 库
target_link_libraries(test_opencv ${OpenCV_LIBS})
```

### 6.4 使用 CMake 构建测试项目

在 **x64 Native Tools Command Prompt for VS 2022** 中执行：

```cmd
cd C:\opencv_test

:: 创建构建目录
mkdir build
cd build

:: 配置项目
cmake -G "Visual Studio 17 2022" -A x64 ^
      -DOpenCV_DIR=C:/opencv/install ^
      ..

:: 编译 Release 版本
cmake --build . --config Release

:: 运行测试程序
Release\test_opencv.exe
```

### 6.5 预期结果

程序运行后应：
1. 控制台输出 OpenCV 版本号
2. 弹出窗口显示蓝色背景图像，带有 "OpenCV Works!" 文字、绿色圆形和红色矩形
3. 按任意键后窗口关闭
4. 在 `build\Release\` 目录生成 `test_output.png` 文件

---

